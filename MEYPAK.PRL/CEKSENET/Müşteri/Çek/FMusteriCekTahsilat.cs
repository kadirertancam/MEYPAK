...
namespace MEYPAK.PRL.CEKSENET.Müşteri.Çek
{
    public partial class FMusteriCekTahsilat : XtraForm
    {
        ...
        void GridiDoldur()
        {
            DGMCekTahsilat.DataSource = _tempCariListe.Select(x => new
            {
                x.BORDRONO,
                x.CEKNO,
                x.TUTAR,
        ...
        {
            _tempCek = null;
            fMusteriCekListe = new FMusteriCekListe(this.Tag.ToString(), "FMusteriCekTahsilat");
        ...
        {
            if (MPKullanici.YetkiGetir(AllForms.MUSTERICEKTAHSILAT.ToString()).EKLE==true)
            {
            ...
            if (_tempCari != null)
            {
                if (_tempCariListe.Count() > 0)
                {
                    ...
                    MessageBox.Show("Çek eklemeden Tahsilat yapamazsınız!");
            }
            else
                MessageBox.Show("Cari Seçmeden Tahsilat yapamazsınız!");
            }
            else
        ...
        {
            _cariListe = new FCariList(this.Tag.ToString(), "FMusteriCekTahsilat");
