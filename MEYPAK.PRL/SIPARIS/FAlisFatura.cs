...
namespace MEYPAK.PRL.SIPARIS
{
    public partial class FAlisFatura : XtraForm
    {
        ...
        {
            _fStokOlcuBrList = new FStokOlcuBrList(this.Tag.ToString(), "FFatura",_tempStok.id);
        ...
        void Doldur()
        {
            if (_tempFatura != null)
            {
                ...
                foreach (var item2 in _faturadetayServis.obje.Where(x => x.faturaid == _tempFatura.id))
                {
                    ...
                    foreach (var item in _stokKasaHarServis.obje.Where(z => z.faturadetayid == item2.id))
                    {
                        kslt = new KasaList()
                        {
