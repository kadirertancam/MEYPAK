...
namespace MEYPAK.PRL.SIPARIS.Raporlar
{
    public partial class FMusteriSiparisRaporu :XtraForm
    {
        ...
        void Doldur()
        {
                ...
                DGMSiparisRaporu.DataSource = _musteriSiparisServis.obje.Where(x => x.kayittipi == 0).Select(x => new
                {
                    ID = x.id,
        ...
        void SiparisKoduDoldur()
        {

            if (_tempSIPARIS != null )
            {
                if (_tempSIPARIS.id > 0)
                {
                    BTSiparisSec.Text = _tempSIPARIS.belgeno.ToString();
