    ...
    public partial class FMusteriSiparis : Form
    {
        ...
        void ToplamHesapla()
        {
            ...
            if (TBAIskonto1.Text != "0" || TBAIskonto1.Text != "" || TBAIskonto2.Text != "0" || TBAIskonto2.Text != "" || TBAIskonto3.Text != "0" || TBAIskonto3.Text != "")
            {
                ...
                TBBrutToplam.EditValue = decimal.Round((_tempSiparisDetay.Sum(x => x.BrütToplam)), 2);
                TBAraToplam.EditValue = decimal.Round(temppte,2);
                TBIskontoToplam.EditValue = (decimal.Round((_tempSiparisDetay.Sum(x => x.BrütToplam)), 2) - temppte);
            ...
            else
            {
                ...
                TBBrutToplam.EditValue = decimal.Round(_tempSiparisDetay.Sum(x => x.BrütToplam), 2);
                TBIskontoToplam.EditValue = decimal.Round(_tempSiparisDetay.Sum(x => x.İskontoTutarı), 2);
                TBKdvTutari.EditValue = decimal.Round(_tempSiparisDetay.Sum(x => x.KdvTutarı), 2);
                TBGenelToplam.EditValue = decimal.Round(_tempSiparisDetay.Sum(x => x.NetToplam + x.KdvTutarı), 2);
                TBAraToplam.EditValue = decimal.Round(_tempSiparisDetay.Sum(x => x.NetToplam), 2);
            }
        ...
        {
            if (MPKullanici.YetkiGetir(AllForms.MUSTERISIPARISTANIM.ToString()).EKLE == true)
            {
                ...
                if (_cariKart.obje.Where(x => x.kod == TBCariKodu.Text).Count() > 0)
                {
                    ...
                    _siparisServis.Data(ServisList.SiparisEkleServis, new PocoSIPARIS()
                    {
                        ...
                        istkontotoplam = Convert.ToDecimal(TBIskontoToplam.EditValue), //_tempSiparisDetay.Sum(x => x.İskontoTutarı),
