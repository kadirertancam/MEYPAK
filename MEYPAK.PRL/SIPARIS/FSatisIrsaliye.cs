...
namespace MEYPAK.PRL.IRSALIYE
{
    public partial class FSatisIrsaliye : Form
    {
        ...
        public FSatisIrsaliye(PocoIRSALIYE _tempIrsaliyes = null, List<PocoIrsaliyeKalem> _tempIrsaliyeDetays = null, List<ListKasaList> tempkasa=null ,int siparisidd=0, int tip = 0)
        {                                                                                                                                                                         
            InitializeComponent();
            ...
            CBBildirimTurleri.EditValue = hKSBildirimTurleri.Body.BaseResponseMessageOf_BildirimTurleriCevap.Sonuc.BildirimTurleri.Select(x => new { ID = x.Id, ADI = x.BildirimTuruAdi }).FirstOrDefault();
        }
        ...
        void ToplamHesapla()
        {
            ...
            if (TBAIskonto1.Text != "0" || TBAIskonto1.Text != "" || TBAIskonto2.Text != "0" || TBAIskonto2.Text != "" || TBAIskonto3.Text != "0" || TBAIskonto3.Text != "")
            {
                foreach (var item in _tempIrsaliyeDetay)
                {
                    ...
                    iskk = decimal.Round((_tempIrsaliyeDetay.Sum(x => x.İskontoTutarı)), 2);
        ...
        void Hesapla(int rowindex)
        {
            if (sy == 0)
            {
                ...
                if (CHBKdvDahil.Checked == false)
                {
                    ...
                    miktar = _tempIrsaliyeDetay.Where(x => x.StokId.ToString() == gridView1.GetRowCellValue(rowindex, "StokId").ToString()).FirstOrDefault().Safi;
                ...
                else
                {
                    ...
                    miktar = _tempIrsaliyeDetay.Where(x => x.StokId.ToString() == gridView1.GetRowCellValue(rowindex, "StokId").ToString()).FirstOrDefault().Safi; //5
                ...
                foreach (var item in _tempIrsaliyeDetay.Where(x => x.StokKodu != ""))
                {
                    if (item.Kdv == 0)
                    {
        ...
        void Doldur()
        {
            if (_tempIrsaliye != null)
            {
                ...
                CBDepo.EditValue = _depoServis.obje.Where(x => x.id == _tempIrsaliye.depoid).FirstOrDefault().depoadi;
                TBKur.Text = _tempIrsaliye.kur.ToString();
                ...
                foreach (var item in _carialthescaricari.obje.Where(x => x.cariid == _tempIrsaliye.cariid))
                {
                ...
                { 
                    foreach (var item2 in _irsaliyeDetayServis.obje.Where(x => x.irsaliyeid == _tempIrsaliye.id))
                    {
                        ...
                        foreach (var item in _stokKasaHarServis.obje.Where(z => z.irsaliyedetayid == item2.id && z.irsaliyeid == item2.irsaliyeid))
                        {
              ...
              //  _irsaliyeDetayServis.Data(ServisList.IrsaliyeDetayListeServis +2 , null, "query=IRSALIYEID=" + _tempIrsaliye.id.ToString() + " AND KAYITTIPI=0");
                ...
                if (irstip == 0)
                _tempIrsaliyeDetay.AddRange(_irsaliyeDetayServis.obje.Where(x=>x.irsaliyeid==_tempIrsaliye.id && x.kayittipi==0).Select(x => new PocoIrsaliyeKalem()
                {
                    id = x.id,
                ...
                TBAIskonto3.EditValue = _tempIrsaliye.altiskonto3;
                sy = 0;
                ToplamHesapla();
                irstip = 0;
            }
        ...
        private void RepositoryItemButtonEdit5_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            if (gridView1.GetFocusedRowCellValue("Tipi") == "STOK")
            {
                ...
                {
                    FStokOlcuBrList aa = new FStokOlcuBrList(this.Tag.ToString(), "FSatisIrsaliye", Convert.ToInt32(gridView1.GetFocusedRowCellValue("StokId")));
                    aa.ShowDialog();
                }
                else
                {
        ...
        private void RepositoryItemButtonEdit3_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            if (gridView1.GetFocusedRowCellValue("Tipi") == "STOK")
            {
                ...
                if (_kasaaa.Where(x => x.num.ToString() == gridView1.FocusedRowHandle.ToString()).Count() > 0)
                {
                    ...
                    fKasaList = new FStokKasaList(this.Tag.ToString(), "FSatisIrsaliye", gridView1.FocusedRowHandle.ToString(), _kasaaa);
                }
                else
                {
                    fKasaList = new FStokKasaList(this.Tag.ToString(), "FSatisIrsaliye", gridView1.FocusedRowHandle.ToString());
                }
                ...
                if (_kasaaa.Where(x => x.num == gridView1.FocusedRowHandle).Count() > 0)
                {
                    ...
                    //   _tempIrsaliyeDetay.Where(x => x.sıra == gridView1.FocusedRowHandle).FirstOrDefault().KasaMiktar = Convert.ToDecimal(_kasaaa.Where(x => x.num == gridView1.FocusedRowHandle).Select(x => x.KasaList.Sum(z => z.MIKTAR)));
                }
                KasaAltBilgiDoldur();
                GCIrsaliye.RefreshDataSource();
            }
        ...
        private void RepositoryItemButtonEdit_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            ...
            if (gridView1.GetFocusedRowCellValue("Tipi") == "STOK")
            {
                ...
                _fStokList = new FStokList(this.Tag.ToString(), "FSatisIrsaliye");
            ...
            else if (gridView1.GetFocusedRowCellValue("Tipi") == "KASA")
            {
                ...
                _fStokKasaList = new FStokKasaList(this.Tag.ToString(), "FSatisIrsaliye");
            ...
            {
                _fHizmetList = new FHizmetList(this.Tag.ToString(), "FSatisIrsaliye");
        ...
        void faturaNoGuncelle()
        {
            ...
            var maxirs= _irsaliyeServis.obje.Max(x => x.belgeno);
            ...
            if (_cariKart.obje.Where(x => x.kod == TBCariKodu.Text).Count() > 0)
            {
                ...
                foreach (var item in _tempIrsaliyeDetay.Where(x => x.StokKodu != "" && x.StokKodu != null).ToList())
                {
                    ...
                    foreach (var test in _kasaaa.Where(x => x.num == item.sıra))
                    {
                        ...
                        if (_stokKasaHarServis.obje.Where(x => x.irsaliyedetayid == _irsaliyeDetayServis.obje2.id).Except(except).Count() > 0)
                            foreach (var sss in _stokKasaHarServis.obje.Where(x => x.irsaliyedetayid == _irsaliyeDetayServis.obje2.id).Except(except))
                            {
                ...
                if (_tempSilinenFaturaDetay.Count() > 0)
                {
                    ...
                    {
                        _irsaliyeDetayServis.Data(ServisList.IrsaliyeDetayListeServis, id: item.id.ToString(), method: System.Net.Http.HttpMethod.Post);
        ...
        {
            if (_tempIrsaliye != null && _tempIrsaliye.kdvdahil == CHBKdvDahil.Checked)
            {
            }
            else
            {
                ...
                sx = 0;
                if (_tempIrsaliye != null)
                    _tempIrsaliye.kdvdahil = CHBKdvDahil.Checked;
            }
        ...
        private void FFatura_Load(object sender, EventArgs e)
        {
            ...
            _seriServis.Data(ServisList.SeriListeServis);
            _seriHarServis.Data(ServisList.SeriHarListeServis);
            foreach (var item in _seriServis.obje.Where(x => x.TIP == 2 ).Select(x => x.SERINO))
            {
            ...
            Doldur();
            CBSoforListesi.EditValue = soforlist.FirstOrDefault();
            CBDorseListesi.EditValue = dorselist.FirstOrDefault();
            CBAracListesi.EditValue = araclist.FirstOrDefault();
            CBBildirimTurleri.EditValue = 196; 
            CBBildirimTurleri.Properties.ForceInitialize();
            CBBildirimTurleri.EditValue = CBBildirimTurleri.Properties.GetKeyValueByDisplayText("Satış");
        }
        ...
        private void BTNSatirSil_Click(object sender, EventArgs e)
        {
            ...
            {
                _tempSilinenFaturaDetay.Add(new PocoIrsaliyeKalem()
                {
                    id = _tempIrsaliyeDetay[gridView1.FocusedRowHandle].id
                }
               );
                _tempIrsaliyeDetay.Remove(_tempIrsaliyeDetay[gridView1.FocusedRowHandle]);
        ...
        {
            FIrsaliyeList ffaturalist = new FIrsaliyeList(this.Tag.ToString(), "FSatisIrsaliye");
