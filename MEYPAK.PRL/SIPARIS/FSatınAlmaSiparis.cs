        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView1.Columns[e.ColumnIndex].Name == "DGVStoKSec")
            {
                ...
                _tempPocokalem = new PocoSiparisKalem()
                {
                    ...
                    Birim = _tempStok.MPSTOKOLCUBRList.Where(x => x.NUM == 1).Select(x => x.MPOLCUBR.ADI).FirstOrDefault().ToString(),
                    KasaAdı = "",
                ...
                DGVFiyatList.DataSource = _tempStok.MPSTOKFIYATLISTList.Select(x => x.MPSTOKFIYATLISTHARList).ToList();
